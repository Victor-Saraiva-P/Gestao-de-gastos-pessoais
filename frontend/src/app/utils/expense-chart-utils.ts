...
export class ChartUtils {
  ...
  // Gera o gráfico de barras a partir das despesas filtradas
  static generateBarChart(filteredBarData: Expense[], canvasId: string): void {
    const canvas = document.getElementById(canvasId) as HTMLCanvasElement;
    if (!canvas) return;
    ...
    filteredBarData.forEach(expense => {
      const date = new Date(expense.data);
      const monthYear = `${date.toLocaleString('default', { month: 'short' })} ${date.getFullYear()}`;
      totals[monthYear] = (totals[monthYear] || 0) + expense.valor;
    });
